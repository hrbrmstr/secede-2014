...
extract_and_cleanup <- function(data, selector, make_numeric=FALSE) {
  data %>%
    html_nodes(selector) %>%
    html_text() %>%
    gsub("^[[:punct:][:space:]]*|[[:punct:][:space:]]*$", "", .) -> x
  if (make_numeric) as.numeric(gsub("[,[:space:]]*", "", x)) else x
}
...
if (!file.exists("data/secede.rda")) {
  ...
  bbc_vote <- read_html("http://www.bbc.com/news/events/scotland-decides/results")
  ...
  secede <- data_frame(
    council = extract_and_cleanup(bbc_vote, ".body-row__cell--council"),
    electorate = extract_and_cleanup(bbc_vote, ".body-row__cell--electorate", TRUE),
    yes = extract_and_cleanup(bbc_vote, ".body-row__cell--yes", TRUE),
    no = extract_and_cleanup(bbc_vote, ".body-row__cell--no", TRUE)
  )
  ...
  secede <- mutate(secede, gone = (yes > no), color = ifelse(gone, "#0065BD", "#CF142B77"))
...
if (!file.exists("data/scotland.json")) {
  GET("http://static.bbci.co.uk/news/1.194.01570/js/data/maps/l/scotland-elections.js",
...
election_map <- readOGR("data/scotland.json", "scotland-elections")
...
mutate(secede, council = factor(council, election_map@data$NAME, ordered=TRUE)) %>%
  arrange(council) -> secede
...
map_df <- fortify(election_map)

# manually associate the map id's with the Council names and vote data
councils <- data.frame(id = as.character(0:(length(election_map@data$NAME)-1)),
                       council = as.character(election_map@data$NAME))

left_join(map_df, councils) %>%
  left_join(secede) -> map_df
...
secede %>%
  ...
  mutate(value=as.numeric(value)) -> secede_m

# get things sorted to the bars display properly

secede_m <- left_join(secede_m, secede, by="council")
secede_m$variable <- factor(secede_m$variable, levels=c("yes", "no"))
secede_m <- arrange(secede_m, no, variable)
secede_m$council <- factor(secede_m$council, unique(secede_m$council))
...
gg1 <- gg1 + geom_text(aes(label=scales::percent(yes/100)), y=0.08, color="white", size=3)
gg1 <- gg1 + geom_text(aes(label=scales::percent(no/100)), y=0.92, color="white", size=3)
gg1 <- gg1 + coord_flip()
gg1 <- gg1 + labs(x="", y="")
gg1 <- gg1 + theme_bw()
...
vote <- arrangeGrob(gg1, gg, ncol=2,
                    top=textGrob("Scotland Votes", gp=gpar(fontsize=20)))
